...
from packages.observability import ArticleCostAuditor
...
class ArticlePipelineOrchestrator:
    ...
    async def run_pipeline(
        ...
        console.print(f"  → Score: [bold green]{score.total_score}/60[/bold green] (Decision: {score.recommendation})")

        if score.total_score < 48 and not force_pass_score:
            console.print(f"[yellow]Score below threshold (48). Moving to backlog. Stopping pipeline.[/yellow]")
        ...
        console.print(f"  → Verified [bold green]{len(research.claims)}[/bold green] technical claims with primary sources.")
        ...
        console.print(f"  → Drafted article: [bold green]'{draft.title}'[/bold green] (v{draft.version})")
        ...
        console.print(f"  → Validated [bold green]{len(val_checks)}[/bold green] embedded code blocks.")
        ...
        article.qa = qa
        console.print(f"  → Technical QA Score: [bold green]{qa.technical_score}/100[/bold green] | Citation Score: [bold green]{qa.citation_score}/100[/bold green]")
        ...
        console.print(f"  → Primary Keyword: [cyan]{seo.primary_keyword}[/cyan] | Slug: [cyan]{seo.slug}[/cyan]")
        ...
        console.print(f"[bold white]Status: [cyan]AWAITING_APPROVAL[/cyan] — Ready for review via `edge review {article_id}`[/bold white]\n")
