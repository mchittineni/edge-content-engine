...
class HumanApprovalGate:
    ...
    def render_and_prompt(article: ArticleRecord) -> ArticleStatus:
        ...
        title_text = f"[bold white]{article.draft.title if article.draft else article.topic}[/bold white]"
        ...
        if article.score:
            table.add_row("Opportunity Score", f"{article.score.total_score}/60", article.score.recommendation)
        if article.qa:
            table.add_row("Technical Accuracy", f"{article.qa.technical_score}%", "✓ Passed" if article.qa.passed else "⚠ Issues flagged")
            table.add_row("Primary Citations", f"{article.qa.citation_score}%", f"{article.qa.verified_claims_count} claims verified")
            table.add_row("Code Syntax & Linting", f"{article.qa.code_validity_score}%", "All snippets tested")
        if article.seo:
        ...
        if article.research and article.research.claims:
            ...
            for idx, claim in enumerate(article.research.claims[:3], 1):
                ...
                console.print(f"     [dim]Source: {claim.source_name} ({claim.tier.value[:6]}) — {claim.source_url}[/dim]")
        ...
        if article.social:
            ...
            console.print(f"  • [blue]LinkedIn[/blue]: Leadership insight ready ({len(article.social.linkedin_post)} chars)")
            console.print(f"  • [red]Reddit[/red]: Authentic discussion ready (Target: {article.social.reddit_post.get('target_subreddits')})")
            console.print(f"  • [white]X Thread[/white]: {len(article.social.x_thread)} tweets synthesized")
        ...
        if choice == "1":
            ...
            if article.draft:
                console.print(Panel(article.draft.full_markdown[:2500] + "\n\n[dim]... (truncated)[/dim]", title="Draft Preview"))
